...
INSERT INTO PhongBan (MaPB, TenPB, ChiNhanh) VALUES ( N'P024',N'Phòng Tuyệt vời HN', N'Hà nội')
GO
...
BEGIN
	...
	FROM NhanVien
	WHERE MaPB in (
	SELECT MaPB FROM PhongBanSG
	)

	SELECT * INTO NhanVienHN
	FROM NhanVien
	WHERE MaPB in (
		SELECT MaPB FROM PhongBanHN
	)
END
Go
...
BEGIN
    ...
    BEGIN
        RAISERROR(N'Các dữ liệu không được để trống', 16, 1)
        RETURN; 
    END
    ...
    BEGIN
        RAISERROR(N'Chi nhánh phải là "Sài gòn" hoặc "Hà nội"', 16, 1)
        RETURN; 
    END

    -- Check MaPB đã tồn tại ở PB SG chưa
	IF EXISTS (SELECT 1 FROM PhongBanSG WHERE MaPB = @MaPB)
		BEGIN
			RAISERROR(N'MaPB đã tồn tại', 16, 1)
			RETURN; 
		END
	-- Check MaPB đã tồn tại ở PB HN chưa
		IF EXISTS (SELECT 1 FROM PhongBanHN WHERE MaPB = @MaPB)
		BEGIN
			RAISERROR(N'MaPB đã tồn tại', 16, 1)
			RETURN; 
		END
    -- Nếu không có vấn đề, thêm dữ liệu
	IF (@ChiNhanh = N'Sài gòn')
	BEGIN
		INSERT INTO PhongBanSG (MaPB, TenPB, ChiNhanh)
					VALUES (@MaPB, @TenPB, @ChiNhanh)
	END
	ELSE IF (@ChiNhanh = N'Hà nội')
	BEGIN
		INSERT INTO PhongBanHN (MaPB, TenPB, ChiNhanh)
		VALUES (@MaPB, @TenPB, @ChiNhanh)
	END
    PRINT N'Thêm dữ liệu thành công'
END

EXEC ThemPB N'P024',N'Phòng Tuyệt vời', N'Sài gòn';
GO
EXEC ThemPB N'P025',N'Phòng Tuyệt vời', N'Hà nội';
GO
SELECT * FROM PhongBan


-- Câu 3: Tạo stored procedure tên SuaPB để sửa dữ liệu của các phân mảnh ngang của bảng PhongBan
...
BEGIN
    ...
    IF (@ChiNhanh NOT IN (N'Sài gòn', N'Hà nội'))
    BEGIN
        PRINT N'Chi nhánh phải là "Sài gòn" hoặc "Hà nội"'
        RETURN; 
    END

    -- CHECK @MaPB exits
    IF NOT EXISTS (SELECT * FROM PhongBanSG WHERE MaPB = @MaPB) AND NOT EXISTS (SELECT * FROM PhongBanHN WHERE MaPB = @MaPB)
    BEGIN
        PRINT N'Không tìm thấy MaPB để sửa dữ liệu'
        RETURN; 
    END

    DECLARE @OldChiNhanh nvarchar(50)
    SELECT @OldChiNhanh = ChiNhanh FROM PhongBanSG WHERE MaPB = @MaPB
	IF (@OldChiNhanh IS NULL)
		SELECT @OldChiNhanh = ChiNhanh FROM PhongBanHN WHERE MaPB = @MaPB

    -- Nếu chi nhánh cũ khác chi nhánh mới thì dời records đi
    IF @OldChiNhanh != @ChiNhanh
    BEGIN
        IF @OldChiNhanh = N'Sài gòn'
            INSERT INTO PhongBanHN SELECT * FROM PhongBanSG WHERE MaPB = @MaPB
        ELSE
            INSERT INTO PhongBanSG SELECT * FROM PhongBanHN WHERE MaPB = @MaPB

        PRINT N'Dữ liệu phòng ban đã được dời sang phân mảnh khác'
    END

    -- Sửa dữ liệu trong phân mảnh cần sửa
    IF @ChiNhanh = N'Sài gòn'
        UPDATE PhongBanSG
...
EXEC SuaPB N'P024',N'Phòng Tuyệt vời HN', N'Hà nội';
GO
EXEC SuaPB N'P025',N'Phòng Tuyệt vời SG', N'Sài gòn';
GO
EXEC SuaPB N'P025',N'Phòng Tuyệt vời SG 1', N'Sài gòn';
GO
...
EXEC TaoPM_Doc_PB
GO
SELECT * FROM PhongBan_Doc1
SELECT * FROM PhongBan_Doc2


-- Câu 5: Hãy viết Stored procedure tên: XemPB_Doc để lập danh sách tất cả phòng ban từ 2 phân mảnh
-- dọc của bảng PhongBan, danh sách lớp gồm 3 cột: MaPB, TenPB, ChiNhanh.

CREATE PROC XemPB_Doc
AS
BEGIN
	SELECT a.MaPB, a.TenPB, b.ChiNhanh FROM PhongBan_Doc1 as a, PhongBan_Doc2 as b WHERE a.MaPB = b.MaPB
END
GO
...
BEGIN
    ...
    -- check @MaPB exists
    IF NOT EXISTS (SELECT 1 FROM PhongBan_Doc1 WHERE MaPB = @MaPB) OR NOT EXISTS (SELECT 1 FROM PhongBan_Doc2 WHERE MaPB = @MaPB)
    BEGIN
        PRINT N'Không tìm thấy MaPB để sửa dữ liệu'
        RETURN; 
    END
...
EXEC SuaPB_Doc N'P024',N'Phòng Tuyệt vời HN', N'Hà nội';
GO
